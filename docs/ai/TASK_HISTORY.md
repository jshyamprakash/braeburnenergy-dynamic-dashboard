...
- overviewDataFlow Config-Panel Refactor (T01–T13): Moved block/layer management from inline edit to WidgetConfigPanel; added per-block color select + per-layer-gap arrow direction (forward|backward|bidirectional|none); seeded canonical 5-layer layout from kosmos_showcase.html; added Reset Page button (mandatory pages only) with auto-save to DB; build verified clean
...
- ADR-051 Backend: SuperAdmin Role + Module Toggle API — organizationId optional on User; seedSuperAdmin() from env; ModuleConfig singleton model (enabled:LicenseModule[]); ModuleService; GET /api/v1/modules (public) + PATCH /api/v1/modules (SuperAdmin JWT); requireSuperAdmin middleware; /api/v1/license deprecated (Deprecation header); 4 integration tests; build clean
- ADR-051 Phase 2 Frontend: licenseSlice switched from /api/v1/license to /api/v1/modules; state simplified to enabled:LicenseModule[]; removed valid/customer/expiresAt; selectLicenseModules + isModuleEnabled() unchanged public API; useLicense hook updated; build clean
- Module Toggle Admin UI: /modules page (SuperAdmin gate, 3 module cards with toggles, Save/Cancel + loading state); useModuleConfig hook (PATCH /api/v1/modules); fetchLicense() re-dispatch on save; Sidebar Shield nav link (SuperAdmin-only); build clean
- ADR-052 Backend — SuperAdmin Passphrase-Derived Auth: passwordHash optional on User; recoveryPublicKey field added; seedSuperAdmin no longer sets password; challengeStore (in-memory Map, 15min TTL); 5 new auth endpoints (superadmin challenge/login, recovery setup/challenge/redeem); RSA-PSS SHA-256 signature verify; SUPERADMIN_PUBLIC_KEY baked in Dockerfile; login() rejects SuperAdmin with helpful redirect; organizationId optional in TokenPayload; build clean
- ADR-052 Frontend — WebCrypto SuperAdmin Login + Admin Recovery: node-forge installed; lib/crypto/derive-keypair.ts (PBKDF2 32-byte seed → forge RSA-2048 + PSS signing); /superadmin-login page (passphrase → challenge → sign → JWT); /recovery page (userId + passphrase → sign → password reset); RecoverySetupSection on profile page (Admin only, derives + saves recoveryPublicKey PEM); SuperAdmin link on /login; build clean
- ADR-052 Tooling — SuperAdmin Passphrase Derive CLI: node-forge + @types/node-forge added to license-cli; superadmin-derive subcommand (PBKDF2 → deterministic RSA-2048 via options.prng getBytesSync, bypasses forge's native Node.js crypto shortcut); outputs public key PEM + docker build --build-arg snippet + .env line; version bumped to 1.2.0; determinism verified (same passphrase → identical PEM across runs); web build clean; derive-keypair.ts updated to same prng pattern for cross-platform consistency
- Production Docker Compose + Deployment Docs: docker-compose.yml api service gains build.args (LICENSE_PUBLIC_KEY, SUPERADMIN_PUBLIC_KEY baked at image build); JWT_SECRET, CORS_ORIGIN, SUPERADMIN_USERNAME/EMAIL added to env blocks; web service gains build.args for NEXT_PUBLIC_API_URL/WS_URL + apps/web/Dockerfile updated with ARG/ENV in builder stage (NEXT_PUBLIC_* were silently ignored at runtime before); docker-compose.dev.yml updated for parity; .env.example and apps/api/.env.example documented with Security + SuperAdmin + Frontend URL sections; YAML validated clean
- ADR-053 SuperAdmin File-Based Login + Runtime Key Rotation: removed passphrase input from /superadmin-login; file upload (PKCS#8 PEM) signs challenge using forge; SystemConfig MongoDB model (system_configs collection) stores public key override; verifySuperAdminChallenge() made async (DB→env fallback via getPublicKey()); PUT /auth/superadmin/rotate-public-key endpoint (SuperAdmin JWT); Key Rotation UI on /admin-management (paste new public key PEM); apiClient.put() added; license-cli export-private-key command (derives same keypair, exports private key mode 600, prints matching public key); version bumped to 1.3.0; both builds clean
- ADR-054 Admin SA-Authorized Offline Recovery: getSARecoveryChallenge() generates challenge + 8-char base32 token; redeemSARecovery() verifies SA RSA-PSS signature then resets password + mustChangePassword=true; GET /auth/recovery/sa-challenge + POST /auth/recovery/sa-redeem routes (public, no auth); /recovery page rewritten with two tabs (Recovery Passphrase + SuperAdmin Recovery); SA Recovery tab shows short token + challenge hex (copy buttons), collects response string + new password; license-cli sa-recovery-sign command (loads PEM file, signs challenge, outputs base64 response string); fixed existing recovery page API URL prefix bug (/api/v1/ removed); both builds clean
- Security Documentation: docs/security/AUTH_FLOWS.md created covering all SA+Admin auth flows (SA login, key rotation, admin creation, forced password change, SA password reset, SA-authorized recovery, self-service recovery, security notes)
- ARCH_SUMMARY Security Section Refresh (T5): Updated SA auth bullet (ADR-052→ADR-053/054) to reflect file-based login, SystemConfig key storage, key rotation endpoint/UI; added Admin account lifecycle bullet (creation/reset/mustChangePassword); added ADR-054 SA-authorized offline recovery description; removed outdated passphrase-in-browser SA derivation text

- ADR-055 BACnet Gateway + ADR-056 EtherNet/IP Gateway (COMPLETE): BacnetGateway + EnipGateway Mongoose models; BacnetGatewayManagerService + EnipGatewayManagerService (dynamic import of node-bacnet/node-ethernet-ip, NATS sensor.raw publish, auto-device-registration); CRUD routes + controllers (/bacnet-gateways, /enip-gateways) with start/stop/test/status/read endpoints; packages/types BacnetGateway+EnipGateway+Create/Update interfaces; useBacnetGateways + useEnipGateways hooks; BacnetGatewayTable/Modal/DetailPanel + EnipGatewayTable/Modal/DetailPanel components; /bacnet-gateways + /enip-gateways pages; /applications/[appId]/bacnet + /enip sub-pages; sidebar entityItems updated (LayoutGrid BACnet + Shield EtherNet/IP); index.ts wired (setNatsClient/setTriggerDispatcher/restoreRunningGateways for both); server.ts routes registered; module stubs in src/types/external-modules.d.ts; both TS builds clean
- ADR-057 System Health API + ADR-058 BE Agent Chat Service (COMPLETE): Backend (Sonnet T01–T13): SystemHealthService aggregating NATS/Redis/BullMQ/MongoDB/5-gateway status (getSystemHealth async); GET /health/system endpoint (Admin+SuperAdmin RBAC, 200 with full health object); BeAgentService with OpenAI-compat adapter (openai npm package, configurable baseURL from AI_CHAT_ENDPOINT env/SystemConfig, stub mode when unset); Socket.io streaming relay (be-agent:token events); POST /be-agent/chat (202 fire-and-forget, be_agent module gate); requireModule() middleware for license gating; 5 gateway managers now export getConnectedCount(); config.ts ai block (endpoint/apiKey/model); .env.example with AI_CHAT_* stubs. Frontend (Haiku T14–T15): useSystemHealth hook (30s refetch) + /system-health page (5 status cards, green/amber/red, Admin+ only); Sidebar nav entry for SuperAdmin; useBeAgentChat hook (Socket.io subscribe/unsubscribe, sendMessage with streaming token collection); BE Agent Config section in /admin-management (endpoint/apiKey/model inputs, Save button, current config display). Both builds clean (backend + web). openai@^4.77.0 added; node-bacnet/node-ethernet-ip moved to optionalDependencies (dynamic import pattern)

- Kosmos Live Data Wiring (COMPLETE): KosmosBeAgentTabPanel wired to useBeAgentChat hook (real streaming via Socket.io, replaces canned setTimeout); typing indicator uses isStreaming; error state shown in chat bubble; send button/input gated by isModuleEnabled('be_agent'); deviceId from BeAgentTabConfig.deviceId passed to sendMessage for contextual AI responses. OverviewMetricCard updated to use useDeviceSnapshot(deviceId) + fieldName from widget config (live value with hardcoded fallback). WidgetConfigPanel: METRIC_WIDGET_TYPES Set added (overviewAnomalyMetric/Load/Egt); deviceId selector and fieldName selector extended to include metric widgets. BeAgentTabConfig.deviceId? field added to types.ts. Build clean.

- Kosmos Overview Dashboard Seed (COMPLETE): apps/api/src/scripts/seed-kosmos-dashboard.ts — depends on seed-poc-demo + seed-module-devices; resolves combustion_ml_engine + be_sense_edge device IDs at runtime; upserts 3 alarm rules (BE-HEALTH-CRITICAL, CD-ANOMALY-HIGH, CD-PRECURSOR-CRITICAL); upserts Dashboard (kosmos-overview-dashboard) with 1 KosmosPage (Overview, layoutSchemaVersion=3) containing 7 pre-wired widgets: overviewBeSense (8 sensors → combustion_ml_engine fields), overviewAnomalyMetric, overviewLoadMetric, overviewEgtMetric, overviewRealtimeChart, overviewDataFlow (static), overviewBeAgentStatus (be_sense_edge); package.json: added seed:module-devices + seed:kosmos-dashboard scripts; build clean.

- Kosmos BeSense Widget Live Sensor Wiring (COMPLETE): OverviewBeSenseWidget wired to useDeviceSnapshot(cfg.deviceId); SensorItem type updated with fieldName?: string; per-sensor live value = snapshot.fields[fieldName] ?? sensor.value; active derived from snapshot field presence; cvValueLive = snapshot.fields[cvFieldName] ?? cfg.cvValue fallback. WidgetConfigPanel: BESENSE_WIDGET_TYPES Set added; deviceId selector extended; fieldName text subField added to ARRAY_FIELD_SPECS.overviewBeSense.sensors; cvFieldName rendered as plain text input. PALETTE_ENTRIES defaultConfig updated with deviceId/cvFieldName keys + fieldName in each default sensor. Build clean.

- Workflow Editor Week 5.4+ — Advanced Features (COMPLETE): Added 3 missing nodes to NodePalette (trigger:webhook, action:updateVariable, transform:dataMapping) with backend handlers already confirmed; added NodeConfigPanel schemas for action:updateVariable (set/increment/decrement/append operations) and transform:dataMapping (mapping-list field type); trigger:webhook schema pre-existed. BulkImportModal: multi-select .json file picker, shape validation ({name,nodes[],edges[]}), conflict detection against existing workflow names with inline rename input, sequential POST per valid file, 3-stage result summary (imported/skipped/error counts). Import button added to WorkflowsTab header. NodePalette search filter: text input filters label/description, clear button, empty state. Build clean.

- ADR-059 Alarm Notification Channels + Escalation Policies (COMPLETE — T01–T15): Backend (Sonnet): NotificationChannel model (email/webhook/in-app config); EscalationPolicy model (dynamic tiers with delay/severity/channels); AlarmNotificationService fire-and-forget dispatch; AlarmEscalationService 60s cron poll with idempotent escalatedTiers[]; 10 CRUD endpoints + Admin+ RBAC; SMTP stub mode. Frontend (Haiku): useAlarmNotifications hook (6 mutations), NotificationChannelModal/EscalationPolicyModal components, /alarm-management page (tabs + tables + CRUD), Sidebar Bell icon + Admin-only filter. Build verified clean (TS + imports).

- POC Demo Seed Script (COMPLETE): apps/api/src/scripts/seed-demo.ts (new, recreates deleted scripts dir); upserts Application (POC Demo/poc-demo), 4 Devices (be_sense_edge+combustion_ml_engine [gateway], water_monitor_01 [gateway], edge_controller_01 [http]), MqttGateway (Demo MQTT Broker, 6 topicMappings), 3 AlarmRules (GAS_LEAK_CRITICAL/HIGH_TEMP_WARNING/COMBUSTION_ANOMALY), Dashboard (POC Overview, layoutSchemaVersion=3); stable seed IDs for idempotency; seed:demo added to apps/api/package.json + iot-platform/package.json; AlarmRule has no orgId field; verified idempotent (2 consecutive runs, identical output, no duplicates)
