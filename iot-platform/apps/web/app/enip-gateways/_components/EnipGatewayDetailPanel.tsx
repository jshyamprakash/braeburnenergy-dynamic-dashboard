    ...
    <div className="fixed inset-0 z-40 bg-black bg-opacity-50">
      <div className="absolute right-0 top-0 h-full w-96 bg-white dark:bg-gray-900 shadow-lg flex flex-col">
        <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">EtherNet/IP Gateway</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <span className="text-gray-500 dark:text-gray-400 block">Port</span>
              <span className="font-medium text-gray-900 dark:text-white">{gateway.port ?? 44818}</span>
            </div>
            <div>
              <span className="text-gray-500 dark:text-gray-400 block">CPU Slot</span>
              <span className="font-medium text-gray-900 dark:text-white">{gateway.slot ?? 0}</span>
            </div>
            <div>
              ...
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {gateway.tags.map((tag, i) => (
                  <div key={i} className="flex justify-between text-xs text-gray-600 dark:text-gray-400 border-b border-gray-100 dark:border-gray-800 pb-1">
                    ...
                    <span className="font-mono text-gray-500">{tag.tagName}{tag.dataType ? ` [${tag.dataType}]` : ''}</span>
                  </div>
                ))}
              </div>
            </div>
