...
export function EnipGatewayTable({ gateways, onEdit, onDelete, onStart, onStop, onTest, onDetail }: Props) {
  if (gateways.length === 0) {
    return (
      <div className="text-center py-12 border rounded-lg border-slate-200 dark:border-slate-700">
        <p className="text-gray-600 dark:text-slate-400">No EtherNet/IP gateways configured</p>
      </div>
    );
  }
        ...
        <tbody>
          {gateways.map((gw, i) => (
            <tr key={gw.id || i}
              ...
              onClick={() => onDetail(gw)}>
              <td className="px-4 py-3 font-medium text-indigo-600 dark:text-blue-400">{gw.name}</td>
              <td className="px-4 py-3 text-sm text-gray-600 dark:text-slate-400">{gw.host}</td>
              <td className="px-4 py-3 text-sm text-gray-600 dark:text-slate-400">{gw.slot ?? 0}</td>
              <td className="px-4 py-3 text-sm text-gray-600 dark:text-slate-400">{gw.tags?.length ?? 0}</td>
              <td className="px-4 py-3 text-sm">
                <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${statusColor(gw.status)}`}>{gw.status}</span>
              </td>
              <td className="px-4 py-3 text-right">
                <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                  <button onClick={(e) => { e.stopPropagation(); onTest(gw); }} title="Test" className="text-yellow-600 hover:text-yellow-700 p-1">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); onEdit(gw); }} title="Edit" className="text-indigo-600 hover:text-indigo-700 p-1">
                    <Edit2 className="h-4 w-4" />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); onDelete(gw.id); }} title="Delete" className="text-rose-600 hover:text-red-700 p-1">
