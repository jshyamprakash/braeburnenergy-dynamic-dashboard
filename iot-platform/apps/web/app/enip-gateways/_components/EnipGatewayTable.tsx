  ...
  if (gateways.length === 0) {
    return (
      <div className="text-center py-12 border rounded-lg border-gray-200 dark:border-gray-700">
        <p className="text-gray-600 dark:text-gray-400">No EtherNet/IP gateways configured</p>
      </div>
    );
  }
        ...
        <tbody>
          {gateways.map((gw, i) => (
            <tr key={gw.id || i}
              ...
              <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{gw.slot ?? 0}</td>
              <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{gw.tags?.length ?? 0}</td>
