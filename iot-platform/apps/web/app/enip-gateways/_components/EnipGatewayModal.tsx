      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">PLC Host / IP *</label>
            <input value={host} onChange={(e) => setHost(e.target.value)} required placeholder="192.168.1.10"
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-slate-800 dark:text-white" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">TCP Port</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">CPU Slot</label>
            <input type="number" min="0" max="15" value={slot} onChange={(e) => setSlot(e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-slate-800 dark:text-white" />
            <p className="text-xs text-slate-500 mt-1">ControlLogix: 0–15, CompactLogix: 0</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Timeout (ms)</label>
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <label className="text-sm font-medium text-gray-700 dark:text-slate-300">CIP Tags</label>
          ...
          <div className="space-y-2 max-h-56 overflow-y-auto">
            {tags.length === 0 && <p className="text-xs text-slate-400 italic">No tags — click "Add Tag".</p>}
            {tags.map((tag) => (
              <div key={tag._tempId} className="flex gap-2 items-center text-xs">
                <input value={tag.tagName} onChange={(e) => updateTag(tag._tempId, 'tagName', e.target.value)}
                  ...
                  className="flex-1 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded dark:bg-slate-800 dark:text-white" />
                <input value={tag.field} onChange={(e) => updateTag(tag._tempId, 'field', e.target.value)}
                  placeholder="Field name"
                  className="w-28 px-2 py-1 border border-gray-300 dark:border-gray-600 rounded dark:bg-slate-800 dark:text-white" />
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
          <button type="button" onClick={onClose} disabled={isLoading}
            className="px-4 py-2 text-gray-700 dark:text-slate-300 bg-gray-100 dark:bg-slate-800 rounded-md hover:bg-gray-200 disabled:opacity-50">
            Cancel
          </button>
