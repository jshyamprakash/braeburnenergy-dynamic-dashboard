  ...
  if (gateways.length === 0) {
    return (
      <div className="text-center py-12 border rounded-lg border-slate-200 dark:border-slate-700">
        <p className="text-gray-600 dark:text-slate-400">No BACnet gateways configured</p>
      </div>
    );
  }
        ...
        <tbody>
          {gateways.map((gw, i) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm text-gray-600 dark:text-slate-400">{gw.port ?? 47808}</td>
              <td className="px-4 py-3 text-sm text-gray-600 dark:text-slate-400">{gw.objects?.length ?? 0}</td>
