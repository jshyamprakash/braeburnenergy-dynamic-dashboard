 ...
 * Run this after seed-poc-demo.ts and after seeding admin user.
 *
 * Devices:
 *   combustion_ml_engine — Combustion DL module (cloud/on-prem ML analytics)
 *   be_sense_edge        — BE AGENT module (edge AI inference)
 *
 * MQTT Topics:
 ...
 *   npx ts-node apps/api/src/scripts/seed-module-devices.ts
 *
...
async function main() {
  ...
  console.log(`✅ Using POC Demo application (${APP_ID})\n`);
  ...
  if (!combustionGateway) {
    await MqttGateway.create({
      ...
      brokerUrl: process.env.COMBUSTION_MQTT_BROKER || 'mqtt://localhost:1883',
  ...
  if (!beSenseGateway) {
    await MqttGateway.create({
      ...
      brokerUrl: process.env.BE_SENSE_MQTT_BROKER || 'mqtt://localhost:1883',
